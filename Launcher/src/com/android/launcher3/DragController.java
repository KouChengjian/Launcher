...
public class DragController {
     ...
     *          Makes dragging feel more precise, e.g. you can clip out a transparent border
     *          
     * 计算要拖拽的view的大小，显示在workspace上，
     * dragView.show(mMotionDownX, mMotionDownY);这个show（）会根据手指的移动而移动的。
     * 然后在通过handleMoveEvent（）方法来分发拖拽的目标到底在哪个目标上。
     * DropTarget一共有3个：workspace，ButtonDropTarget（删除类），
     * Folder；他们分别实现了DropTarget这个接口。
     */
    public void startDrag(Bitmap b, int dragLayerX, int dragLayerY,
        ...
        dragView.show(mMotionDownX, mMotionDownY); // 移动到哪显示在哪
        handleMoveEvent(mMotionDownX, mMotionDownY); //拖拽的目标到底在哪个目标上
    }
    ...
    private void handleMoveEvent(int x, int y) {
        ...
        DropTarget dropTarget = findDropTarget(x, y, coordinates); // 判断在哪个拖拽目标里面
...
//        Log.e("TAG", dragLayerX +"   "+dragLayerY+"====");
